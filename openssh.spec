...
%define openssh_rel 100
%define pam_ssh_agent_ver 0.9.5
...
%define __spec_install_post \
    ...
    %{__arch_install_post} \
    %{__os_install_post} \
    fipshmac $RPM_BUILD_ROOT%{_libdir}/fipscheck $RPM_BUILD_ROOT%{_bindir}/ssh $RPM_BUILD_ROOT%{_sbindir}/sshd \
%{nil}
...
%post server
#enable PAM
sed -i -e 's/#UsePAM no/UsePAM yes/i' /etc/ssh/sshd_config
sed -i -e 's/UsePAM no/UsePAM yes/i' /etc/ssh/sshd_config
/usr/bin/ssh-keygen -t ecdsa -f /etc/ssh/ssh_host_ecdsa_key -N ''
/sbin/restorecon -R /etc/ssh >/dev/null 2>&1
#add service
